...
[지난 글](/posts/git-worktree/)에서 Worktree를 살펴봤다.

하나의 저장소에서 여러 작업 디렉토리를 동시에 운용하는 방식이었다.

그런데 작업 디렉토리가 여러 개라면, 혹은 브랜치가 여러 개라면 — 자연스럽게 이런 질문이 생긴다:

**"이 두 파일, 뭐가 다르지?"**

`git diff`. 매일 쓰는 명령어다. 하지만 어떻게 두 파일의 차이를 찾는 걸까?
...
가로축을 A의 줄, 세로축을 B의 줄로 놓으면:

<pre class="mermaid">
graph TD
    subgraph "Edit Graph (A→B)"
        direction LR
        p00["(0,0)"] -->|"→ 삭제 c"| p10["(1,0)"]
        p10 -->|"→ 삭제 b"| p20["(2,0)"]
        p20 -->|"→ 삭제 c"| p30["(3,0)"]
        p00 -->|"↓ 삽입 a"| p01["(0,1)"]
        p01 -->|"↓ 삽입 b"| p02["(0,2)"]
